...
render_clients = async function(){
  ...
  clients.forEach((c, idx) => {
    ...
    if(box && !box.querySelector('.delete-client-btn')){
      box.insertAdjacentHTML('beforeend', `<button class="btn btn-bad btn-xs delete-client-btn" onclick="deleteClient(${c.id})">Excluir</button>`);
    }
  });
};
...
clientView = async function(id){
  ...
  if(head && !head.querySelector('.delete-client-btn')){
    ...
    actions.insertAdjacentHTML('beforeend', `<button class="btn btn-bad delete-client-btn" onclick="deleteClient(${c.id})">Excluir cliente</button>`);
  }
};
