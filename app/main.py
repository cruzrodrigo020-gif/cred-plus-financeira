...
def parse_date(value: str, field_name: str = 'data') -> date:
    ...
    raise HTTPException(
        status_code=400,
        detail=f'{field_name.capitalize()} inválida. Use DD-MM-AAAA, DD/MM/AAAA ou AAAA-MM-DD.',
    )


def make_token(u):
    return jwt.encode(
        {'sub': str(u.id), 'role': u.role, 'exp': datetime.utcnow() + timedelta(hours=8)},
        SECRET,
        algorithm='HS256',
    )
...
app = FastAPI(title='CRED+ Financeira API', version='10.2.0')
app.mount('/static', StaticFiles(directory='app/static'), name='static')


@app.exception_handler(Exception)
async def unexpected_error(request: Request, exc: Exception):
    # Mantém a resposta consumível pelo painel e evita texto puro "Internal Server Error".
...
def dashboard(authorization: Optional[str] = Header(None), s: Session = Depends(db)):
    ...
    scope = q.all()
    receivable = sum(max(0, i.amount - i.paid_amount) for i in scope if i.status != 'paid')
    overdue = sum(
    ...
    received_today = sum(p.amount for p in pq.filter(Payment.payment_date == today).all())
    ...
    cq = s.query(Client)
    kq = s.query(Contract).filter_by(status='active')
    if u.role == 'collector':
        cq = cq.filter(Client.collector_id == u.id)
        kq = kq.filter(Contract.collector_id == u.id)
    return {
...
def clients(authorization: Optional[str] = Header(None), s: Session = Depends(db)):
    ...
    return [
        ...
        for x in q.order_by(Client.name).all()
    ]
...
def products(authorization: Optional[str] = Header(None), s: Session = Depends(db)):
    ...
    return [
        {'id': x.id, 'name': x.name, 'rate': x.rate, 'days': x.days, 'periodicity': x.periodicity}
    ...
    return [
        {
            'id': x.id,
            'number': x.number,
            'client': s.get(Client, x.client_id).name,
            'principal': x.principal,
            'total': x.total,
            'installments': x.installments,
            'first_due': str(x.first_due),
            'status': x.status,
        }
        for x in q.order_by(Contract.id.desc()).all()
    ]
...
def create_contract(
    ...
    number = 'CTR-' + datetime.now().strftime('%y%m%d%H%M%S%f')[-10:]
    ...
    for i in range(1, n + 1):
        s.add(
            Installment(
                contract_id=x.id,
                number=i,
                due_date=due + timedelta(days=i - 1),
                amount=value,
            )
        )
    s.add(
    ...
    s.commit()
    log(s, u, 'CONTRACT_CREATE', number)
        ...
        out.append(
            {
                ...
                'contract': c.number,
                'client': s.get(Client, c.client_id).name,
                'number': i.number,
...
def edit_installment(
    ...
    u = auth(authorization, s)
    admin(u)
    i = s.get(Installment, iid)
    if not i:
        raise HTTPException(404, 'Parcela não encontrada')
    old = i.due_date
    i.due_date = parse_date(due_date, 'vencimento')
    s.commit()
    log(s, u, 'DUE_DATE_CHANGE', f'{iid}: {old} -> {i.due_date}')
    return {'ok': True}
...
def payment(
    ...
    if amount <= 0 or amount > remaining + 0.005:
        raise HTTPException(400, 'Valor de pagamento inválido')

    paid = parse_date(payment_date, 'data do pagamento')
    i.paid_amount += amount
    i.paid_at = paid
    i.status = 'paid' if i.paid_amount >= i.amount - 0.005 else 'partial'
    ...
    s.commit()
    log(s, u, 'PAYMENT_CREATE', f'{c.number}/{i.number}')
    return {'ok': True}
...
def expense(
    ...
    s.add(
        Cash(
            ...
            category='Despesa',
            amount=amount,
            movement_date=parse_date(movement_date, 'data da despesa'),
            description=description,
            user_id=u.id,
        )
    )
    s.commit()
    log(s, u, 'EXPENSE', str(amount))
    return {'ok': True}
...
def users(authorization: Optional[str] = Header(None), s: Session = Depends(db)):
    ...
    admin(u)
    return [
        {'id': x.id, 'name': x.name, 'username': x.username, 'role': x.role, 'active': x.active}
...
def create_user(
    ...
    x = User(name=name, username=username, password_hash=pwd.hash(password), role=role)
    s.add(x)
    s.commit()
    log(s, u, 'USER_CREATE', username)
    return {'id': x.id}
